    ...
    <div className="h-full w-full flex items-center justify-center bg-[var(--surface-base)] p-6 overflow-y-auto">
      <div className="w-full max-w-3xl space-y-8 animate-in fade-in zoom-in-95 duration-300">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-[var(--neutral-12)]">
            {hasConnections ? "Pick a database to get started" : "Welcome to QueryDen"}
          </h1>
          <p className="text-sm text-[var(--neutral-11)] max-w-md mx-auto">
            {hasConnections
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="text-center space-y-2">
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs text-[var(--neutral-11)] max-w-md">
...
function HasConnectionsActions() {
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-center gap-2 text-xs text-[var(--neutral-11)] opacity-70 pt-2">
...
function LauncherCard({
  ...
  return (
    <button
      onClick={onClick}
      className="group text-left p-4 rounded-xl bg-[var(--surface-elevated)] border border-[var(--neutral-6)] hover:border-[var(--accent-9)]/50 hover:bg-[var(--surface-elevated)] transition-all"
    >
      <div className={`${accent} mb-2 transition-transform group-hover:scale-110 origin-left`}>
        {icon}
      </div>
      <div className="text-sm font-bold text-[var(--neutral-12)] mb-0.5">{title}</div>
      <div className="text-xs text-[var(--neutral-11)]">{description}</div>
    </button>
  );
}
