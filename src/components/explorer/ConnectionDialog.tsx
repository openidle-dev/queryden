...
export function ConnectionDialog({ connection, onClose, defaultFolderId }: { connection?: DatabaseConnection; onClose: () => void; defaultFolderId?: string }) {
  ...
  const testConnection = async (): Promise<{ success: boolean; message: string }> => {
    ...
    try {
      ...
      // Two traps. `close()` with no argument closes *every* pool in the
      // process, so testing a connection used to drop every other connection
      // the user had open. And because pools are reused by connection string,
      // this handle may *be* a live connection's pool, which is not ours to
      // close at all.
