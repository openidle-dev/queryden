...
/**
 * Build an `AND <column> IN (...)` fragment restricting a query to the schemas
 * the user selected, or an empty string when they selected none (meaning "all").
 *
 * Schema names are identifiers the user controls and are interpolated rather
 * than bound, so single quotes are doubled. A schema really can contain one:
 * `CREATE SCHEMA "o'brien"` is legal.
 */
export function schemaInClause(column: string, schemas: readonly string[]): string {
