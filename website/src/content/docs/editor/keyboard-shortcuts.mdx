...
The default "DataGrip" preset includes bindings for features still being wired (e.g. `Shift Shift` global search, `Ctrl+Shift+C` console, `Ctrl+B` go-to-definition, `Shift+F6` rename, `Ctrl+PageUp/PageDown` tab cycling). These are tracked in [issue #11](https://github.com/openidle-dev/queryden/issues/11).

> Shortcuts that conflict with operating system behavior (such as Ctrl+W closing the window, Ctrl+\ sending SIGQUIT on Linux, or F4 being captured by macOS) are avoided. The keymap uses Ctrl+Shift+E for the Database Explorer and Ctrl+PageUp/PageDown for tab navigation to prevent these conflicts.
